...
import { BookOpen, ChevronLeft, Coffee, Library, Plus, Save, Star, Trash2, Users } from 'lucide-react';
      ...
      <main className="px-4 space-y-4">
        ...
        {showForm ? (
          <section className="rounded-[28px] bg-card/70 border border-white/[0.07] p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setShowForm(false)} className="h-12 rounded-full bg-white/[0.06] border border-white/[0.08] text-foreground text-[13px] font-semibold">Отмена</button>
              <button onClick={save} disabled={!canSave} className="h-12 rounded-full bg-primary disabled:bg-muted disabled:text-muted-foreground text-primary-foreground text-[13px] font-semibold flex items-center justify-center gap-2">
                <Save size={15} />
                Сохранить
              </button>
            </div>
        ...
        <section className="space-y-3 pt-2">
          ...
          {books.length === 0 ? (
            ...
            books.map((book) => (
              <BookCard key={book.id} book={book} onDelete={() => deleteBook(book.id)} onToggleFavorite={() => updateBook(book.id, { favorite: !book.favorite })} />
            ))
          )}
        </section>
      </main>
...
function BookCard({ book, onDelete, onToggleFavorite }: { book: BookRating; onDelete: () => void; onToggleFavorite: () => void }) {
  return (
    <article className="rounded-[28px] bg-card/70 border border-white/[0.07] overflow-hidden">
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-white/[0.06]">
          ...
          <button onClick={onDelete} className="w-9 h-9 rounded-full bg-red-950/35 border border-red-800/30 text-red-300 flex items-center justify-center">
            <Trash2 size={15} />
          </button>
        </div>
      </div>
