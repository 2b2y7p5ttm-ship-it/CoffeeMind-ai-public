    ...
    <AnimatePresence>
      {tasting && (
        <motion.div className="cm-preview-layer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
          ...
          >
            <button className="cm-preview-close" onClick={onClose}><X size={18} /></button>
            <p className="cm-preview-kicker">Быстрый взгляд</p>
            <h2>{tasting.coffeeName}</h2>
            <p className="cm-preview-origin">{[tasting.country, tasting.region, tasting.processing || tasting.process].filter(Boolean).join(' · ')}</p>
            <div className="cm-preview-score"><strong>{tasting.overallScore}</strong><span>Общая оценка</span></div>
            ...
            <button className="cm-primary-action w-full justify-center mt-6" onClick={onOpen}>Открыть главу</button>
          </motion.article>
        </motion.div>
      )}
